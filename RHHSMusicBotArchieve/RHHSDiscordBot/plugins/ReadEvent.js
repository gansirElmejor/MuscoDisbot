const FilePath = "../../../var/www/html/datas/onGoingEvent.json";
const fs = require('fs');
...
function GetEventDate(DateStr) {
    ...
    if ((IptDay-1)%10==0) {
        DateSuffix = "st";
    } else if ((IptDay-2)%10==0) {
        DateSuffix = "nd";
    } else if ((IptDay-3)%10==0) {
        DateSuffix = "rd";
    } else {
        DateSuffix = "th";
    }
    
    var finalStr = MouthStrConvert_Array[IptMonth]+" "+(IptDay.toString()+DateSuffix)+" "+(IptHour.toString())+":"+(IptMinute.toString());
    return finalStr;
} catch(err) {
    console.log(err);
    return "Error";
