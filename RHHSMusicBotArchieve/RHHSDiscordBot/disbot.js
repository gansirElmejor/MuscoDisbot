...
const MakeAnnouncement = require("./plugins/Announcement.js");
const FeelBored = require("./plugins/Bored.js");
const YodaTranslate = require("./plugins/TranslateYoda.js");
// ChatPlugins Above
...
function PostEvent_Proceed(User_Inst,msgStr,channel) {
    ...
    function WriteToFile(AllInputs) {
        try {
             fs.readFile('../../var/www/html/datas/onGoingEvent.json', 'utf8', function readFileCallback(err, data){
            if (err){
                console.log(err);
                return false;
            } else {
                ...
                fs.writeFile('../../var/www/html/datas/onGoingEvent.json', json, 'utf8', (err) => err && console.error(err));
                return true;
        }});
          return true;
        } catch(Err) {
...
client.on("messageCreate", async function(message){
    try{
        ...
        if (Perfix == CommandPerfix+"announce") {
            if (!(AdministratorId.includes(User_Inst.id))) {
                message.react("❌");
                message.reply("Sorry, you are not authenticated to run this command!")
                return;
            ...
            MakeAnnouncement(client,message,User_Inst);
            return;
        } else
        if (Perfix == CommandPerfix+"bored" || Perfix == CommandPerfix+"boring") {
            FeelBored(message);
            return;
        } else
        if (Perfix == CommandPerfix+"yoda") {
            if (typeof SplitedContent[1] == 'undefined') {
                message.react("❌");
                message.reply("Hey, you cannot let yoda translate blank.");
                return;
            };
            let Arg1 = message.content.substring(6,message.content.length);
            YodaTranslate(message,Arg1);
            return;
        }
